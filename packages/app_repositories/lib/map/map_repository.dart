...
abstract class MapRepository {
  ...
  Future<List<MapPlace>> places(
      {String? country, String? category, String search = '', int offset = 0,bool savedOnly=false});
...
class ApiMapRepository implements MapRepository {
  ...
  Future<List<MapCountry>> countries({String? userId}) async =>
      apiObjectList(await _client.get(userId==null?'map/countries':'map/profiles/$userId/countries'))
  ...
  Future<MapPlaceDetail> detail(String id) async {
    ...
    final signed=await ApiMediaSigner(_client).signMany(previews.map((p)=>p['media_url'] as String? ?? ''));
    for(final preview in previews){preview['media_url']=signed[preview['media_url']] ?? '';}
