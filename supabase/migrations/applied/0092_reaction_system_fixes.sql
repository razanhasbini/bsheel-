-- ============================================================
-- MIGRATION 0092: Reaction system fixes
-- 1. Block self-votes at the DB level (trigger)
-- 2. Add composite index for vote aggregation
-- 3. Add get_reaction_counts RPC (replaces client-side counting)
-- 4. Add vote counts to get_submission_detail (eliminates N+1)
-- ============================================================

-- ── 1. Prevent self-votes ───────────────────────────────────────
...
DROP TRIGGER IF EXISTS prevent_self_reaction_trigger ON public.reactions;
CREATE TRIGGER prevent_self_reaction_trigger
  BEFORE INSERT OR UPDATE ON public.reactions
  FOR EACH ROW EXECUTE FUNCTION public.prevent_self_reaction();

-- ── 2. Composite index ──────────────────────────────────────────
CREATE INDEX IF NOT EXISTS idx_reactions_submission_type
  ON public.reactions (submission_id, type);

-- ── 3. get_reaction_counts RPC ─────────────────────────────────
CREATE OR REPLACE FUNCTION public.get_reaction_counts(p_submission_id uuid)
RETURNS TABLE (upvote_count bigint, downvote_count bigint)
AS $$
  SELECT
...
GRANT EXECUTE ON FUNCTION public.get_reaction_counts(uuid) TO authenticated, anon;

-- ── 4. get_submission_detail with vote counts ──────────────────
...
BEGIN
  ...
  RETURN QUERY
    ...
    LEFT JOIN (
      SELECT
        submission_id AS sid,
        count(*) FILTER (WHERE type = 'upvote') AS ups,
        count(*) FILTER (WHERE type = 'downvote') AS downs
      FROM public.reactions
