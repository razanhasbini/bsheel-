-- ============================================================
-- MIGRATION 0060: Fix comment_reply spam (M2)
-- Require caller to also have a comment on the same submission.
-- ============================================================
...
BEGIN
  ...
  IF p_type = 'comment_reply' AND v_reference_id IS NOT NULL THEN
    ...
    END IF;
    -- Caller must also have a comment on the same submission
    IF NOT EXISTS (
      SELECT 1 FROM public.comments
      WHERE submission_id = v_reference_id::uuid
