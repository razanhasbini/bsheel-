-- ============================================================
-- MIGRATION 0090: Add p_sort parameter to get_feed RPC
--
-- Enables server-side sorting for feed tabs:
--   'recent'       → submitted_at DESC (default)
--   'top'          → net_score DESC
--   'hot'          → hot_score DESC
--   'bottom'       → net_score ASC
--   'graveyard'    → hot_score ASC, submitted_at ASC
-- ============================================================
...
BEGIN
  RETURN QUERY
    ...
    ORDER BY
      CASE p_sort
        WHEN 'top'       THEN NULL  -- handled below
        WHEN 'hot'       THEN NULL
        WHEN 'bottom'    THEN NULL
        WHEN 'graveyard' THEN NULL
        ELSE NULL
      END,
