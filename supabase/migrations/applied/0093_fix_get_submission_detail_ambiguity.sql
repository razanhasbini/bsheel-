-- ============================================================
-- MIGRATION 0093: Fix ambiguous "submission_id" in
-- get_submission_detail (introduced by 0092).
--
-- The RETURNS TABLE columns become PL/pgSQL variables that
-- shadow unqualified column names. The vote-counts subquery
-- referenced `submission_id` without a table alias, causing:
--   column reference "submission_id" is ambiguous (42702)
-- Fix: alias `reactions` as `rx` and qualify both references.
-- ============================================================
