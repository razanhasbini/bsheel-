-- ============================================================
-- MIGRATION 0073: Feed RPC with collab group fields (v2)
-- ============================================================
...
BEGIN
  RETURN QUERY
    SELECT
      ...
      -- JSON array of all group members with their submissions + votes
    ...
    WHERE s.status = 'approved'
      ...
      -- Dedup: for group quests, only show the creator's submission.
      -- All member data is in the collab_members JSON array.
      AND (
        gm.group_id IS NULL                              -- solo quest
        OR gm.user_id = g.creator_id                     -- group: only creator's row
      )
