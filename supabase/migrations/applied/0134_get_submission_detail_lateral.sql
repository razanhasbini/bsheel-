-- ============================================================
-- MIGRATION 0134: Replace global reactions aggregate with a LATERAL
-- correlated subquery in get_submission_detail (ARC-018).
--
-- The previous body grouped EVERY row in public.reactions then joined
-- on the single submission we care about. That O(N) full-table scan
-- ran on every post-detail open + every realtime tick.
--
-- The LATERAL form filters reactions to the submission first, so the
-- planner only touches the rows that matter.
-- ============================================================
...
BEGIN
  ...
  RETURN QUERY
    ...
    -- ARC-018: was a full-table aggregate-then-join on reactions.
    -- LATERAL filters by submission_id first so we never aggregate
    -- rows we'll discard.
    LEFT JOIN LATERAL (
      SELECT
