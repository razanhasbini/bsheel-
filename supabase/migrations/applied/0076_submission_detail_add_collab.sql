-- ============================================================
-- MIGRATION 0076: Add collab fields to get_submission_detail RPC
-- Mirrors collab fields from get_feed (0075) so that a tapped
-- collab post in the feed resolves to the full group detail
-- (all members + all their submissions) instead of a single
-- user's submission.
-- ============================================================
...
BEGIN
  -- If this submission belongs to a collab group, resolve to the
  -- group leader's submission so detail view always anchors on the
  -- same record regardless of which member was tapped in the feed.
