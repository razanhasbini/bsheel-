-- Canonical body for get_submission_detail. ARC-018 / 0134.
-- Per-row LATERAL aggregate on reactions instead of full-table.
...
BEGIN
  ...
  RETURN QUERY
    SELECT
      s.id, s.media_url, s.media_type, s.caption, s.submitted_at,
      p.id, p.username, p.display_name, p.avatar_url, p.bio,
      q.id, q.title, q.description, q.category, q.xp_reward,
      s.show_in_feed, s.visibility,
      (gm.group_id IS NOT NULL),
      gm.group_id, g.mode,
      COALESCE(mc.cnt, 0)::bigint,
      CASE WHEN gm.group_id IS NOT NULL THEN (
        SELECT json_agg(json_build_object(
          'user_id', mp.id, 'username', mp.username,
          'display_name', mp.display_name, 'avatar_url', mp.avatar_url,
          'bio', mp.bio, 'submission_id', ms.id, 'media_url', ms.media_url,
          'media_type', ms.media_type, 'submission_status', ms.status,
          'caption', ms.caption, 'show_in_feed', ms.show_in_feed,
        ...
        LEFT JOIN (
          ...
          FROM public.collab_votes cv
          WHERE cv.group_id = gm.group_id AND cv.voter_id = v_uid
        ) mv ON mv.sid = ms.id
        WHERE m2.group_id = gm.group_id
      ) ELSE NULL END,
      COALESCE(r.ups, 0)::bigint,
      COALESCE(r.downs, 0)::bigint,
      (COALESCE(r.ups, 0) - COALESCE(r.downs, 0))::bigint,
      uq.expires_at
    FROM public.submissions s
