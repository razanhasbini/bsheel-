...
// Type and glyphs float directly on the media, so they carry a soft dark
// shadow for legibility on bright frames. 54% black, as the original reels
// player drew it.
const Color _legibilityShadow =
    Color(0x8A000000); // Screen-specific colour — not a theme token.
const List<Shadow> _textShadows = <Shadow>[
  Shadow(color: _legibilityShadow, blurRadius: 8, offset: Offset(0, 1)),
];
const List<Shadow> _inkTextShadows = <Shadow>[
  Shadow(color: QuestColors.pureBlack, blurRadius: 8, offset: Offset(0, 1)),
];

/// Instagram Reels-style full-screen post — the feed as it shipped in the
/// legacy Bsheel app.
///
...
/// in full, letterboxed on black.
///
/// The card is presentation only: the host resolves the viewer's vote and
/// save state, the comment count and every callback, and hands them in as
/// plain values.
class ReelsCard extends ConsumerStatefulWidget {
  ...
  final String? myVoteType; // ReactionType.upvote | downvote | null
  final bool isSaved;
  ...
  /// Collab/versus context — when [collabMode] is versus and
...
class _ReelsCardState extends ConsumerState<ReelsCard> {
  // Bottom legibility scrim stops — a fade from clear to ~85% black.
  ...
  Widget build(BuildContext context) {
    ...
    final isCollab = (widget.collabMode == CollabMode.versus ||
            widget.collabMode == CollabMode.with_) &&
        _members.isNotEmpty;
...
class _ReelsMediaItemState extends ConsumerState<_ReelsMediaItem> {
  ...
  Future<void> _initVideo() async {
    ...
    try {
      ...
      // The placeholder plate stays up; nothing else to do for a broken URL.
    }
  }
                        ...
                        child: Opacity(
                          ...
                          child: Transform.scale(
                            ...
                            child: const Icon(
                              ...
                              shadows: [
                                Shadow(
                                    color: _legibilityShadow, blurRadius: 12),
                              ],
                            ),
                          ),
                        ),
  ...
  Widget _buildImage(BoxConstraints constraints) {
    ...
    if (isLandscape && _imageProvider != null) {
      // Letterbox on solid black, the way reels apps do — the media owns
      // the visual frame and nothing competes with it.
      return Stack(
        fit: StackFit.expand,
        children: [
          const ColoredBox(color: QuestColors.pureBlack),
  ...
  Widget _buildVideo(BoxConstraints constraints) {
    ...
    if (v == null || !_videoReady) {
      // Black plate behind the loader so it reads on every device.
      return const Stack(
        fit: StackFit.expand,
        children: [
...
/// initialising or buffering mid-stream. On a soft dark plate so it reads
/// against bright media too.
      ...
      child: SizedBox(
        ...
        child: CircularProgressIndicator(
          ...
          valueColor: AlwaysStoppedAnimation<Color>(
              QuestColors.pureWhite.withAlpha(178)),
        ),
      ),
    );
  }
    ...
    return ColoredBox(
      ...
      child: Center(
        child: Icon(Icons.image_not_supported_rounded,
            color: QuestColors.pureWhite.withAlpha(61), size: 48),
      ),
    );
  }
}
    ...
    return Column(
      ...
      children: [
        ...
        _RailButton(
          ...
          label: '',
          semanticLabel: isSaved ? 'Saved to BSHEEEL' : 'Save to BSHEEEL',
...
class _RailButtonState extends State<_RailButton> {
  ...
  Widget build(BuildContext context) {
    ...
    // legibility on bright media. Colour only flips when the action is
    // ACTIVE (upvoted → red arrow, saved → gold bookmark).
    final iconColor =
...
class _BottomMeta extends StatelessWidget {
  ...
  bool get _isVersus => collabMode == CollabMode.versus;
        ...
        if (modeBadge != null && modeBadge!.isNotEmpty) ...[
          Container(
            ...
            child: Text(
              ...
              style: TextStyle(
                // Ink on the green badge, white on the red one — whichever
                // measures readable on that ground.
                color: QuestColors.onAccent(_isVersus
                    ? QuestColors.softRed
                    : (collabMode == CollabMode.with_
                        ? QuestColors.successGreen
                        : QuestColors.softRed)),
            ...
            children: [
              ...
              Text(
                ...
                style: const TextStyle(
                  ...
                  fontWeight: FontWeight.w800,
                  letterSpacing: 0.4,
                  shadows: [Shadow(color: _legibilityShadow, blurRadius: 6)],
                ),
              ),
              const SizedBox(height: 6),
              Text(
