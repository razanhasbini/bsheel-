...
class _JoinCollabPageState extends ConsumerState<JoinCollabPage> {
  static const Color _inkShadow20 = Color(0x331A1330); // Screen-specific colour — not a theme token.
  static const Color _inkShadow15 = Color(0x261A1330); // Screen-specific colour — not a theme token.
  ...
  Future<void> _join() async {
    ...
    try {
      final result = await ref.read(collabRepositoryProvider).joinGroup(widget.code);
      ...
      if (mounted) {
        ScaffoldMessenger.of(context)
            .showSnackBar(const SnackBar(content: Text('Joined the group quest!')));
    ...
    return await showDialog<bool>(
          ...
          builder: (ctx) => AlertDialog(
            ...
            title: Text('ABANDON CURRENT QUEST?',
                style: QuestTypography.headlineSmall.copyWith(color: QuestColors.text(context))),
            content: Text('You have an active quest. Abandon it to join this group.',
                style: QuestTypography.bodyMedium.copyWith(color: QuestColors.textDim(context))),
            actions: [
              TextButton(
                  onPressed: () => Navigator.of(ctx).pop(false),
                  child: Text('CANCEL', style: TextStyle(color: QuestColors.text(context)))),
              TextButton(
          ...
          error: (e, _) => Center(
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  const Icon(Icons.error_outline, size: 48, color: QuestColors.softRed),
                  const SizedBox(height: QuestSpacing.md),
                  Text('GROUP NOT FOUND',
                      style: QuestTypography.headlineSmall.copyWith(color: navyColor)),
                  const SizedBox(height: QuestSpacing.sm),
                  Text('This group may have expired or is full.',
                      style: QuestTypography.bodyMedium.copyWith(color: navyColor.withAlpha(160)),
                  ...
                  GestureDetector(
                    ...
                    child: Container(
                      padding: const EdgeInsets.symmetric(
                          horizontal: QuestSpacing.xl, vertical: QuestSpacing.md),
...
class _Chip extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Text(label,
          style: QuestTypography.labelSmall.copyWith(color: color, fontSize: 10, letterSpacing: 1)),
    );
  }
}
