...
> ⚠️ **These steps are obsolete and are kept only as a record of what this
> audit shipped.** They predate the move off Supabase Cloud. There is no
> Supabase-cloud project any more, so `supabase db push` and
> `supabase functions deploy` have nothing to talk to.
>
> **The current process is `docs/DEPLOYMENT.md`: open a PR, merge to `main`,
> and the "Deploy to server" workflow applies pending migrations and rsyncs
> the edge functions to the self-hosted box.** Never SSH in to apply these
> by hand. Only steps 3-5 below still involve a human, and step 3 is now
> the only genuinely manual deploy in the project.

```bash
# 1. New SQL migrations  — NOW AUTOMATIC on merge to main.
#    (was: cd supabase && npx supabase db push)
#    0140, 0141, 0142 have long since been applied and frozen in
#    supabase/migrations/applied/.

# 2. Edge functions      — NOW AUTOMATIC on merge to main.
#    (was: npx supabase functions deploy <name>)
#    deploy-server.yml rsyncs supabase/functions/ and restarts the runtime.

# 3. Cloudflare R2 worker — STILL MANUAL. Not touched by CI, and it lives
#    on a different Cloudflare account. See cloudflare/DEPLOY_UPLOAD_WORKER.md.
ALLOWED_UPLOAD_ORIGINS=https://admin.bsheel.app \
  npx wrangler deploy --config cloudflare/wrangler-upload.toml

# 4. Mobile app — STILL MANUAL (App Store / Play release, not a CI deploy).
...
# 5. Owner-side actions (see "Owner action items" below)
```
...
| H11 | Floating Deno imports | ✅ Fixed | all 10 functions pinned to `@supabase/supabase-js@2.47.10` |
...
| M10 | Profile `.select('*')` | ✅ Fixed | column allow-list in `supabase_profile_repository.dart` |
...
8. **Set `ALLOWED_UPLOAD_ORIGINS`** env var on the Cloudflare R2 worker before re-deploy (M4).
9. **Set `ADMIN_ALLOWED_ORIGINS`** env var on `send-push` and `admin_manage_user` Supabase functions (existing; verify).

## What changed in the repo (file inventory)

```
supabase/migrations/0140_security_audit_2026_05_17.sql  (new)
supabase/migrations/0141_gdpr_export.sql                (new)
supabase/migrations/0142_age_gate_and_consent.sql       (new)

supabase/functions/admin_manage_user/index.ts           (M)
supabase/functions/send-push/index.ts                   (M)
supabase/functions/telegram-webhook/index.ts            (M)
supabase/functions/{telegram-*, notify-on-insert}/index.ts (M — pinned imports)

cloudflare/worker-r2-upload.js                          (M — magic bytes, quota, CORS)
...
packages/app_repositories/lib/profile/supabase_profile_repository.dart (M — column allow-list)
