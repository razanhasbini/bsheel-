...
Phone sign-in has its own copies on the **API host**, generated by the API
itself (`PhoneSigninLandingController`), because an association file only
counts for links on the host that serves it and the callback is
`https://api.bsheel.app/phone-signin-callback`. Those three paths sit at the
domain root, outside `/api/v1`, so the `api.bsheel.app` Caddy block needs the
`@bsheel_root` handle that `deploy/caddy-apply.sh` adds (idempotent; live
since 2026-09-12). Without it the site's catch-all answers all three with the
literal string `Bsheel API` — a 200, and completely wrong: iOS never learns
the app owns the link and a verified tester dead-ends on that string.
`deploy/verify.sh` checks both.
