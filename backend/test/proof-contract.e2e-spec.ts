...
describe('quest verification contract (e2e)', { timeout: 120_000 }, () => {
  ...
  // system.
  it('fails closed for a category with no default', async () => {
    const quest = await harness.createQuest({ category: 'category-that-has-no-default' });
  ...
  // Every fixture quest in the whole e2e suite uses category 'e2e', which
  // matches no seeded default. So the fail-closed path is not a corner case
  // here — it is what every other test in the suite is already relying on,
  // and it is why none of them can accidentally give the agent authority.
  it("resolves the harness's own default category to no authority", async () => {
