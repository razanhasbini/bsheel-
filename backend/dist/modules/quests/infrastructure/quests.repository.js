...
let QuestsRepository = class QuestsRepository {
    database;
    constructor(database) {
        this.database = database;
    }
    async findQuest(id) {
        const result = await this.database.query('SELECT * FROM quests WHERE id = $1', [id]);
        return result.rows[0] ?? null;
    }
    async listAll() {
        return (await this.database.query('SELECT * FROM quests ORDER BY created_at DESC, id DESC')).rows;
    }
    async create(input, actorId) {
        const result = await this.database.query(`INSERT INTO quests (title, description, category, difficulty, xp_reward, duration_hours, is_active, created_by)
    ...
    async createBulk(input, actorId) {
        return this.database.transaction(async (transaction) => {
            const result = await transaction.query(`INSERT INTO quests
         ...
         RETURNING *`, [
                JSON.stringify(input.map((quest) => ({
                    title: quest.title,
         ...
         VALUES ($1, 'quest.bulk_create', 'quest', $2::jsonb)`, [actorId, JSON.stringify({ count: result.rowCount })]);
            return result.rows;
        });
    }
    async update(id, input) {
        const result = await this.database.query(`UPDATE quests SET
    ...
    async delete(id, actorId) {
        try {
            await this.database.transaction(async (transaction) => {
                const quest = await transaction.query('DELETE FROM quests WHERE id = $1 RETURNING *', [id]);
                if (!quest.rows[0]) {
                ...
                await transaction.query(`INSERT INTO admin_audit_log
             (actor_id, action, target_type, target_id, before_state)
           VALUES ($1, 'quest.delete', 'quest', $2, $3::jsonb)`, [actorId, id, JSON.stringify(quest.rows[0])]);
            });
        }
        catch (error) {
            if (this.isForeignKeyViolation(error)) {
                throw new ConflictException({
                    code: 'QUEST_IN_USE',
                    message: 'Quest is still referenced by collaboration or scheduling data',
                });
            }
            throw error;
        }
    }
    async deleteAll(actorId) {
        try {
            return await this.database.transaction(async (transaction) => {
                const deleted = await transaction.query('DELETE FROM quests RETURNING id');
                await transaction.query(`INSERT INTO admin_audit_log (actor_id, action, target_type, after_state)
           VALUES ($1, 'quest.delete_all', 'quest', $2::jsonb)`, [actorId, JSON.stringify({ deleted: deleted.rowCount })]);
            ...
            if (this.isForeignKeyViolation(error)) {
                throw new ConflictException({
                    code: 'QUESTS_IN_USE',
                    message: 'One or more quests are still referenced by collaboration or scheduling data',
                });
            }
            throw error;
        }
    ...
    async findActiveForUser(userId) {
        const result = await this.database.query(`SELECT uq.*, row_to_json(q.*) AS quests
       FROM user_quests uq JOIN quests q ON q.id = uq.quest_id
       WHERE uq.user_id = $1 AND uq.status IN ('assigned', 'submitted')
       ORDER BY uq.assigned_at DESC LIMIT 1`, [userId]);
        return result.rows[0] ?? null;
    }
    ...
    async assignSpecific(userId, questId) {
        return this.database.transaction(async (transaction) => {
            ...
            const questResult = await transaction.query('SELECT * FROM quests WHERE id = $1 AND is_active = true', [questId]);
            ...
            const assigned = await transaction.query(`INSERT INTO user_quests (user_id, quest_id, expires_at)
         VALUES ($1, $2, now() + make_interval(hours => $3)) RETURNING *`, [userId, questId, quest.duration_hours]);
            await transaction.query(`UPDATE admin_quest_injections SET consumed_at = now()
         WHERE target_user_id = $1 AND quest_id = $2 AND consumed_at IS NULL`, [userId, questId]);
            await this.insertAssignmentNotification(userId, assigned.rows[0], quest.duration_hours, transaction);
            await this.emitQuest('quest.assigned', assigned.rows[0].id, {
                userId,
    ...
    async expire(userId, userQuestId) {
        await this.database.transaction(async (transaction) => {
            const result = await transaction.query(`UPDATE user_quests SET status = 'expired', version = version + 1
         WHERE id = $1 AND user_id = $2 AND status = 'assigned' AND expires_at <= now()
         RETURNING id`, [userQuestId, userId]);
            if (!result.rowCount) {
                throw new ConflictException({
            ...
            await this.emitQuest('quest.expired', userQuestId, {
                userId,
                userQuestId,
            }, transaction);
        });
    }
    async pickerOptions(userId, requestedCount) {
        const count = Math.min(Math.max(requestedCount || 3, 1), 20);
        return this.database.transaction(async (transaction) => {
            await this.lockUser(userId, transaction);
            const injection = await transaction.query(`WITH popped AS (
           ...
           RETURNING quest_id
         ) SELECT q.* FROM popped p JOIN quests q ON q.id = p.quest_id WHERE q.is_active`, [userId]);
            const injected = injection.rows[0];
            const remaining = count - (injected ? 1 : 0);
            if (remaining <= 0)
                return injected ? [injected] : [];
            const adminSlots = Math.max(1, Math.ceil(remaining / 2));
            const result = await transaction.query(`WITH eligible AS (
         ...
         ) SELECT * FROM preferred UNION ALL SELECT * FROM filler`, [userId, injected?.id ?? null, adminSlots, remaining]);
            return injected ? [injected, ...result.rows] : result.rows;
        });
    }
    async questOfTheDay() {
        const result = await this.database.query(`SELECT d.id, to_char(d.display_date, 'YYYY-MM-DD') AS display_date, d.ticket_no, d.bonus_xp, q.id AS quest_id,
    ...
    async followingActive(userId, limit) {
        const capped = Math.min(Math.max(limit || 12, 1), 40);
        const result = await this.database.query(`WITH followed AS (
         ...
         ORDER BY pool, assigned_at DESC LIMIT $2`, [userId, capped]);
        return result.rows;
    }
    async rerollsRemaining(userId) {
        const result = await this.database.query(`SELECT GREATEST(0, 5 - count(*))::integer AS remaining FROM quest_reroll_log
       WHERE user_id = $1 AND rerolled_at > now() - interval '24 hours'`, [userId]);
        return result.rows[0].remaining;
    }
    async recordReroll(userId) {
        return this.database.transaction(async (transaction) => {
            await this.lockUser(userId, transaction);
            const count = await transaction.query(`SELECT count(*)::integer AS used FROM quest_reroll_log
         WHERE user_id = $1 AND rerolled_at > now() - interval '24 hours'`, [userId]);
            ...
            await transaction.query('INSERT INTO quest_reroll_log (user_id) VALUES ($1)', [userId]);
            return 5 - count.rows[0].used - 1;
        });
    }
    async lockUser(userId, transaction) {
        await transaction.query('SELECT pg_advisory_xact_lock(hashtextextended($1, 0))', [userId]);
    }
    async expireOverdueForUser(userId, transaction) {
        const expired = await transaction.query(`UPDATE user_quests SET status = 'expired', version = version + 1
       WHERE user_id = $1 AND status = 'assigned' AND expires_at < now()
       RETURNING id`, [userId]);
        for (const assignment of expired.rows) {
            await this.emitQuest('quest.expired', assignment.id, {
                userId,
                userQuestId: assignment.id,
            }, transaction);
        }
    }
    async insertAssignmentNotification(userId, assignment, durationHours, transaction) {
        const count = await transaction.query('SELECT count(*)::integer AS count FROM user_quests WHERE user_id = $1', [userId]);
       ...
       VALUES ($1, $2, $3, 'quest_assigned', $4) RETURNING id`, [userId, title, body, assignment.id]);
    ...
    async emitQuest(eventType, aggregateId, payload, transaction) {
        await transaction.query(`INSERT INTO outbox_events (aggregate_type, aggregate_id, event_type, payload)
       VALUES ('quest', $1, $2, $3::jsonb)`, [aggregateId, eventType, JSON.stringify(payload)]);
    }
...
QuestsRepository = __decorate([
    Injectable(),
    __metadata("design:paramtypes", [DatabaseService])
], QuestsRepository);
export { QuestsRepository };
//# sourceMappingURL=quests.repository.js.map