...
let SearchRepository = class SearchRepository {
    ...
    async search(viewerId, query, limit, offset) {
        const escaped = query.replaceAll('\\', '\\\\').replaceAll('%', '\\%').replaceAll('_', '\\_');
        const pattern = `%${escaped}%`;
        const [users, quests, posts] = await Promise.all([
            this.database.query(`SELECT p.id, p.username::text, p.display_name, p.avatar_url, p.bio, p.xp, p.level,
         ...
         LIMIT $3 OFFSET $4`, [viewerId, pattern, limit, offset]),
            this.database.query(`SELECT id, title, description, category, difficulty, xp_reward, duration_hours,
         ...
         LIMIT $2 OFFSET $3`, [pattern, limit, offset]),
            this.database.query(`SELECT s.id, s.user_quest_id, s.user_id, s.media_url, s.media_type::text,
         ...
         LIMIT $3 OFFSET $4`, [viewerId, pattern, Math.min(limit, 24), offset]),
        ]);
