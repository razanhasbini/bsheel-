...
let SubmissionsRepository = class SubmissionsRepository {
    ...
    async create(userId, input) {
        return this.database.transaction(async (transaction) => {
            const userQuest = await transaction.query('SELECT status::text, expires_at FROM user_quests WHERE id = $1 AND user_id = $2 FOR UPDATE', [input.userQuestId, userId]);
            const assignment = userQuest.rows[0];
            if (!assignment)
                throw new NotFoundException({ code: 'USER_QUEST_NOT_FOUND', message: 'Quest assignment not found' });
            ...
            const media = await transaction.query(`SELECT object_key, content_type FROM media_objects
         WHERE user_id = $1 AND kind = 'submission' AND status = 'ready'
           AND object_key = ANY($2::text[])`, [userId, mediaKeys]);
            ...
            const result = await transaction.query(`INSERT INTO submissions (user_quest_id, user_id, media_url, media_type, caption, show_in_feed)
         VALUES ($1, $2, $3, $4, $5, $6) RETURNING *`, [input.userQuestId, userId, input.mediaUrl, input.mediaType, input.caption?.trim() || null, input.showInFeed]);
            await transaction.query("UPDATE user_quests SET status = 'submitted', version = version + 1 WHERE id = $1", [input.userQuestId]);
            await transaction.query(`UPDATE collab_group_members m SET submission_time_seconds = GREATEST(0,
           floor(EXTRACT(EPOCH FROM ($2::timestamptz - uq.assigned_at)))::integer)
         FROM user_quests uq WHERE m.user_quest_id = $1 AND uq.id = m.user_quest_id`, [input.userQuestId, result.rows[0].submitted_at]);
    ...
    async findVisibleToUser(id, viewerId) {
        const result = await this.database.query(`SELECT s.*, q.id AS quest_id, q.title AS quest_title, q.description AS quest_description,
    ...
    async listUser(userId, viewerId, limit, offset) {
        const result = await this.database.query(`SELECT s.* FROM submissions s
       WHERE s.user_id = $1 AND s.visibility <> 'deleted' AND s.deleted_at IS NULL
         AND ($1 = $2 OR s.status = 'approved')
       ORDER BY s.submitted_at DESC, s.id DESC LIMIT $3 OFFSET $4`, [userId, viewerId, Math.min(Math.max(limit, 1), 100), Math.max(offset, 0)]);
        return result.rows;
    }
    async adminDetail(id) {
        const result = await this.database.query(`SELECT s.*,
    ...
    async reviewQueue(limit, offset) {
        const result = await this.database.query(`WITH queue AS (
       ...
       ORDER BY queue.submitted_at ASC, queue.id`, [Math.min(Math.max(limit, 1), 100), Math.max(offset, 0)]);
        return result.rows;
    }
    async listForAdmin(filter) {
        ...
        const result = await this.database.query(`SELECT s.*, p.username::text, p.display_name, p.avatar_url,
       ...
       LIMIT ${limitPlaceholder} OFFSET ${offsetPlaceholder}`, parameters);
        return result.rows;
    }
    async appeal(userId, id, appealNote) {
        await this.database.transaction(async (transaction) => {
            const result = await transaction.query('SELECT * FROM submissions WHERE id = $1 FOR UPDATE', [id]);
            const submission = result.rows[0];
            if (!submission)
                throw new NotFoundException({ code: 'SUBMISSION_NOT_FOUND', message: 'Submission not found' });
            if (submission.user_id !== userId)
                throw new ForbiddenException({ code: 'NOT_SUBMISSION_OWNER', message: 'Not your submission' });
            if (submission.status !== 'rejected')
                throw new ConflictException({ code: 'SUBMISSION_NOT_REJECTED', message: 'Only rejected submissions can be appealed' });
            if (submission.appealed)
                throw new ConflictException({ code: 'ALREADY_APPEALED', message: 'Already appealed' });
            if (submission.visibility === 'deleted')
                throw new ConflictException({ code: 'DELETED_SUBMISSION', message: 'Cannot appeal a deleted submission' });
            await transaction.query(`UPDATE submissions SET status = 'pending', appeal_note = $2, appealed = true,
           reviewed_by = NULL, review_note = NULL, reviewed_at = NULL, version = version + 1
         WHERE id = $1`, [id, appealNote.trim()]);
    ...
    async approve(actorId, id, reviewNote, source = {}) {
        await this.database.transaction(async (transaction) => {
            const submission = await this.lockForReview(id, transaction);
            const profile = await transaction.query('SELECT level FROM profiles WHERE id = $1 FOR UPDATE', [submission.user_id]);
            const oldLevel = profile.rows[0].level;
            const updated = await transaction.query(`UPDATE submissions SET status = 'approved', reviewed_by = $2, reviewed_at = now(),
           review_note = COALESCE(NULLIF(trim($3), ''), review_note), xp_awarded = true,
           xp_awarded_amount = $4, version = version + 1
         WHERE id = $1 RETURNING *`, [id, actorId, reviewNote ?? null, submission.quest_xp]);
            ...
            const newProfile = await transaction.query(`UPDATE profiles SET xp = xp + $2, quests_completed = quests_completed + 1,
           level = GREATEST(1, (xp + $2) / 100 + 1)
         WHERE id = $1 RETURNING level`, [submission.user_id, submission.quest_xp]);
            ...
            if (newProfile.rows[0].level > oldLevel) {
                await this.createNotification(submission.user_id, `Level ${newProfile.rows[0].level}. Look at you. 🎮`, "You levelled up in real life. That's not a thing most apps can say.", 'level_up', null, transaction);
            }
    ...
    async reject(actorId, id, reviewNote, source = {}) {
        await this.database.transaction(async (transaction) => {
            const submission = await this.lockForReview(id, transaction);
            const updated = await transaction.query(`UPDATE submissions SET status = 'rejected', reviewed_by = $2, review_note = $3,
           reviewed_at = now(), version = version + 1 WHERE id = $1 RETURNING *`, [id, actorId, reviewNote.trim()]);
            ...
            await this.createNotification(submission.user_id, deleted ? 'Quest rejected. We took the post down.' : 'Quest rejected. Try again. 🔁', deleted
                ? 'Mods pulled the post and rolled back the XP. No drama. The quest is yours again whenever you want it.'
                : reviewNote.trim() || "The judges weren't convinced this round. Take another swing. Same quest, fresh shot.", 'submission_rejected', id, transaction);
    ...
    async setVisibility(userId, id, visibility) {
        await this.database.transaction(async (transaction) => {
            await this.transitionVisibility(id, visibility, transaction, userId);
        });
    }
    async removeByAdmin(actorId, id, reason) {
        await this.database.transaction(async (transaction) => {
    ...
    async transitionVisibility(id, visibility, transaction, ownerId) {
        const locked = await transaction.query(`SELECT * FROM submissions WHERE id = $1
       AND ($2::uuid IS NULL OR user_id = $2) FOR UPDATE`, [id, ownerId ?? null]);
        const submission = locked.rows[0];
        if (!submission)
            throw new NotFoundException({ code: 'SUBMISSION_NOT_FOUND', message: 'Submission not found' });
        if (submission.visibility === visibility)
            return { submission, xpRolledBack: 0 };
        ...
        if (xpRolledBack > 0 || (visibility === 'deleted' && submission.xp_awarded)) {
            await transaction.query(`UPDATE profiles SET
           xp = GREATEST(0, xp - $2),
           quests_completed = GREATEST(0, quests_completed - 1),
           level = GREATEST(1, GREATEST(0, xp - $2) / 100 + 1)
         WHERE id = $1`, [submission.user_id, xpRolledBack]);
        }
        await transaction.query(`UPDATE submissions SET
    ...
    async lockForReview(id, transaction) {
        const result = await transaction.query(`SELECT s.*, q.xp_reward AS quest_xp FROM submissions s
       JOIN user_quests uq ON uq.id = s.user_quest_id JOIN quests q ON q.id = uq.quest_id
    ...
    async notifyAdmins(submission, appeal, transaction) {
        const author = await transaction.query(`SELECT COALESCE(NULLIF(display_name, ''), username::text, 'Someone') AS name FROM profiles WHERE id = $1`, [submission.user_id]);
        ...
        const notifications = await transaction.query(`INSERT INTO notifications (user_id, title, body, type, reference_id)
       SELECT user_id, $1, $2, $3, $4 FROM admins RETURNING id, user_id`, [title, body, type, submission.id]);
    ...
    async createNotification(userId, title, body, type, referenceId, transaction) {
        const result = await transaction.query(`INSERT INTO notifications (user_id, title, body, type, reference_id)
       VALUES ($1, $2, $3, $4, $5) RETURNING id`, [userId, title, body, type, referenceId]);
        await this.emit('notification', result.rows[0].id, 'notification.created', { notificationId: result.rows[0].id, userId }, transaction);
    }
    async notifyCollabPartners(userId, userQuestId, submissionId, transaction) {
        const group = await transaction.query('SELECT group_id FROM collab_group_members WHERE user_quest_id = $1', [userQuestId]);
        if (!group.rows[0])
            return;
        const author = await transaction.query(`SELECT COALESCE(NULLIF(display_name, ''), username::text, 'Someone') AS name
       FROM profiles WHERE id = $1`, [userId]);
        const notifications = await transaction.query(`INSERT INTO notifications (user_id, title, body, type, reference_id, actor_id)
    ...
    async emit(aggregateType, aggregateId, eventType, payload, transaction) {
        await transaction.query(`INSERT INTO outbox_events (aggregate_type, aggregate_id, event_type, payload)
       VALUES ($1, $2, $3, $4::jsonb)`, [aggregateType, aggregateId, eventType, JSON.stringify(payload)]);
    }
    async audit(actorId, action, targetId, afterState, transaction) {
        await transaction.query(`INSERT INTO admin_audit_log (actor_id, action, target_type, target_id, after_state)
       VALUES ($1, $2, 'submission', $3, $4::jsonb)`, [actorId, action, targetId, JSON.stringify(afterState)]);
    }
};
