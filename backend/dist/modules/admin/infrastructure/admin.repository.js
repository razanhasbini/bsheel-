...
let AdminRepository = class AdminRepository {
    ...
    async me(userId) {
        return ((await this.database.query(`SELECT a.id, a.user_id, a.role::text, p.username::text, p.display_name, p.avatar_url
       FROM admins a JOIN profiles p ON p.id = a.user_id WHERE a.user_id = $1`, [userId])).rows[0] ?? null);
    }
    async stats() {
        return (await this.database.query(`SELECT
         ...
         (SELECT count(*)::integer FROM reports WHERE status = 'pending') AS "pendingReports"`)).rows[0];
    }
    async users(query, limit, offset) {
        const pattern = query
            ? `%${query.replaceAll('\\', '\\\\').replaceAll('%', '\\%').replaceAll('_', '\\_')}%`
            : null;
        return (await this.database.query(`SELECT p.id, p.username::text, p.display_name, p.avatar_url, p.bio, p.xp, p.level,
       ...
       ORDER BY p.created_at DESC, p.id DESC LIMIT $2 OFFSET $3`, [pattern, limit, offset])).rows;
    }
    async createUser(actorId, input) {
        try {
            return await this.database.transaction(async (transaction) => {
                const user = await transaction.query(`INSERT INTO users (email, password_hash, email_verified_at)
           VALUES ($1, $2, now()) RETURNING id`, [input.email.trim().toLowerCase(), input.passwordHash]);
           ...
           VALUES ($1, 'password', $2::text, $2::citext)`, [userId, input.email.trim().toLowerCase()]);
                await this.audit(actorId, 'user.create', 'profile', userId, null, {
           ...
           VALUES ('user', $1, 'user.created', jsonb_build_object('userId', $1::uuid))`, [userId]);
                return { userId };
            });
        }
        catch (error) {
    ...
    async queueUserDeletion(actorId, userId) {
        if (actorId === userId) {
        ...
        await this.database.transaction(async (transaction) => {
            const user = await transaction.query('SELECT status::text FROM users WHERE id = $1 FOR UPDATE', [userId]);
            if (!user.rows[0])
                throw new NotFoundException({
                    code: 'USER_NOT_FOUND',
                    message: 'User not found',
                });
            const request = await transaction.query(`INSERT INTO account_delete_requests (user_id, execute_after)
         VALUES ($1, now()) ON CONFLICT (user_id) DO UPDATE
         SET cancelled_at = NULL, execute_after = now() RETURNING id`, [userId]);
            await transaction.query(`UPDATE profiles SET username = 'deleted_' || substr(replace(id::text, '-', ''), 1, 8),
           display_name = 'Deleted account', avatar_url = NULL, bio = NULL,
           analytics_consent_at = NULL, updated_at = now() WHERE id = $1`, [userId]);
            await transaction.query(`UPDATE users SET status = 'deletion_pending', token_version = token_version + 1,
           updated_at = now() WHERE id = $1`, [userId]);
            ...
            await transaction.query(`INSERT INTO outbox_events (aggregate_type, aggregate_id, event_type, payload)
         VALUES ('account', $1, 'account.deletion.requested', $2::jsonb)`, [userId, JSON.stringify({ requestId: request.rows[0].id, userId })]);
            await this.audit(actorId, 'user.delete', 'profile', userId, {
                status: user.rows[0].status,
            }, { queued: true }, transaction);
        });
    }
    async setAdminRole(actorId, userId, role) {
        if (actorId === userId) {
            throw new BadRequestException({
        ...
        await this.database.transaction(async (transaction) => {
            const target = await transaction.query('SELECT 1 FROM users WHERE id = $1 FOR UPDATE', [userId]);
            if (!target.rowCount)
                throw new NotFoundException({
                    code: 'USER_NOT_FOUND',
                    message: 'User not found',
                });
            const before = await transaction.query('SELECT role::text FROM admins WHERE user_id = $1', [userId]);
            if (role) {
                await transaction.query(`INSERT INTO admins (user_id, role) VALUES ($1, $2::admin_role)
           ON CONFLICT (user_id) DO UPDATE SET role = EXCLUDED.role`, [userId, role]);
            ...
            await this.audit(actorId, role ? 'admin.grant' : 'admin.revoke', 'profile', userId, { role: before.rows[0]?.role ?? null }, { role: role ?? null }, transaction);
        });
    }
    ...
    async forceResetPassword(actorId, userId, passwordHash) {
        if (actorId === userId) {
            throw new BadRequestException({
        ...
        await this.database.transaction(async (transaction) => {
            const target = await transaction.query(`UPDATE users SET password_hash = $2, token_version = token_version + 1, updated_at = now()
         WHERE id = $1 RETURNING email::text`, [userId, passwordHash]);
            if (!target.rows[0])
                throw new NotFoundException({
                    code: 'USER_NOT_FOUND',
                    message: 'User not found',
            ...
            await transaction.query(`INSERT INTO auth_identities (user_id, provider, provider_subject, provider_email)
         VALUES ($1, 'password', $2::text, $2::citext) ON CONFLICT DO NOTHING`, [userId, target.rows[0].email]);
            await transaction.query('UPDATE refresh_sessions SET revoked_at = now() WHERE user_id = $1 AND revoked_at IS NULL', [userId]);
            const notification = await transaction.query(`INSERT INTO notifications (user_id, title, body, type, reference_id)
         VALUES ($1, 'Your password was reset by an admin',
                 'If you did not request this, contact support immediately.',
                 'security_alert', $2) RETURNING id`, [userId, actorId]);
            await transaction.query(`INSERT INTO outbox_events (aggregate_type, aggregate_id, event_type, payload)
         VALUES ('notification', $1, 'notification.created', $2::jsonb)`, [
            ...
            await this.audit(actorId, 'user.reset_password', 'profile', userId, null, {
                forced: true,
            }, transaction);
        });
    }
    async queuePasswordRecovery(actorId, userId, token) {
        await this.database.transaction(async (transaction) => {
            const target = await transaction.query(`SELECT email::text FROM users
         WHERE id = $1 AND deleted_at IS NULL FOR UPDATE`, [userId]);
            if (!target.rows[0]) {
                throw new NotFoundException({
                    code: 'USER_NOT_FOUND',
                    message: 'User not found',
         ...
         RETURNING id`, [userId, token.tokenHash, token.encryptedToken, token.expiresAt]);
            await transaction.query(`INSERT INTO outbox_events (aggregate_type, aggregate_id, event_type, payload)
         VALUES ('auth_action_token', $1, 'auth.password_recovery.requested',
                 jsonb_build_object('tokenId', $1::uuid))`, [action.rows[0].id]);
            await this.audit(actorId, 'user.request_password_reset', 'profile', userId, null, { queued: true }, transaction);
        });
    }
    async setStatus(actorId, userId, status, reason) {
        await this.database.transaction(async (transaction) => {
            ...
            await transaction.query(`UPDATE users SET status = $2::account_status,
    ...
    async setXp(actorId, userId, xp, level, completed, reason) {
        await this.database.transaction(async (transaction) => {
            ...
            await transaction.query('UPDATE profiles SET xp = $2, level = $3, quests_completed = $4, updated_at = now() WHERE id = $1', [userId, xp, level, completed]);
            await this.audit(actorId, 'user.set_xp', 'profile', userId, before.rows[0], { xp, level, quests_completed: completed, reason }, transaction);
        });
    }
    async reports(status, limit, offset) {
        return (await this.database.query(`SELECT r.*, json_build_object('username', p.username::text, 'display_name', p.display_name) AS profiles,
    ...
    async reviewReport(actorId, id, status, note) {
        await this.database.transaction(async (transaction) => {
            const before = await transaction.query('SELECT status::text, admin_note FROM reports WHERE id = $1 FOR UPDATE', [id]);
            if (!before.rows[0]) {
                throw new NotFoundException({
            ...
            await transaction.query(`UPDATE reports SET status = $2, admin_note = COALESCE($3, admin_note), reviewed_by = $4, reviewed_at = now()
         WHERE id = $1`, [id, status, note?.trim() || null, actorId]);
            await this.audit(actorId, 'report.review', 'report', id, before.rows[0], { status, admin_note: note?.trim() || before.rows[0].admin_note }, transaction);
            await transaction.query(`INSERT INTO outbox_events (aggregate_type, aggregate_id, event_type, payload)
         VALUES ('report', $1, 'report.reviewed', $2::jsonb)`, [id, JSON.stringify({ reportId: id, status })]);
        });
    }
    async injections(limit, offset) {
        return (await this.database.query(`SELECT i.*, q.title AS quest_title, p.username::text AS target_username
       FROM admin_quest_injections i JOIN quests q ON q.id = i.quest_id JOIN profiles p ON p.id = i.target_user_id
       WHERE i.consumed_at IS NULL ORDER BY i.created_at DESC LIMIT $1 OFFSET $2`, [limit, offset])).rows;
    }
    async inject(actorId, input) {
        try {
            return await this.database.transaction(async (transaction) => {
                const target = await transaction.query('SELECT 1 FROM profiles WHERE id = $1', [input.targetUserId]);
                if (!target.rowCount)
                    throw new NotFoundException({
                        code: 'PROFILE_NOT_FOUND',
                        message: 'Target user not found',
                    });
                const quest = await transaction.query(`INSERT INTO quests (title, description, category, difficulty, xp_reward, duration_hours, is_active, created_by)
           VALUES ($1, $2, $3, $4, $5, $6, true, $7) RETURNING *`, [
                ...
                await transaction.query('INSERT INTO admin_quest_injections (target_user_id, quest_id, created_by) VALUES ($1, $2, $3)', [input.targetUserId, quest.rows[0].id, actorId]);
                await this.audit(actorId, 'quest.inject', 'user', input.targetUserId, null, { quest_id: quest.rows[0].id, xp_reward: input.xpReward }, transaction);
                return quest.rows[0];
            });
        }
        catch (error) {
    ...
    async cancelInjection(actorId, id) {
        const result = await this.database.query(`UPDATE admin_quest_injections SET consumed_at = now() WHERE id = $1 AND consumed_at IS NULL RETURNING target_user_id`, [id]);
        if (!result.rows[0])
            throw new NotFoundException({
                code: 'INJECTION_NOT_FOUND',
                message: 'Pending injection not found',
            });
        await this.database.query(`INSERT INTO admin_audit_log (actor_id, action, target_type, target_id, after_state)
    ...
    async notify(actorId, targetUserId, title, body, type) {
        return this.database.transaction(async (transaction) => {
            const result = await transaction.query(`INSERT INTO notifications (user_id, title, body, type, actor_id)
         SELECT id, $2, $3, $4, $5 FROM users WHERE status = 'active' AND ($1::uuid IS NULL OR id = $1)
         RETURNING id, user_id`, [targetUserId ?? null, title, body, type, actorId]);
           ...
           FROM notifications WHERE id = ANY($1::uuid[])`, [result.rows.map((row) => row.id)]);
            }
            await this.audit(actorId, targetUserId ? 'notification.send' : 'notification.broadcast', 'notification', null, null, {
    ...
    async setConfig(actorId, key, value, description, isPublic) {
        return (await this.database.query(`INSERT INTO app_config (key, value, description, is_public, updated_by)
       ...
       RETURNING *`, [key, JSON.stringify(value), description ?? null, isPublic, actorId])).rows[0];
    }
    async qotd(limit, offset) {
        return (await this.database.query(`SELECT d.id, d.quest_id, to_char(d.display_date, 'YYYY-MM-DD') AS display_date,
       ...
       ORDER BY d.display_date DESC LIMIT $1 OFFSET $2`, [limit, offset])).rows;
    }
    async setQotd(actorId, input) {
        return (await this.database.query(`INSERT INTO quest_of_the_day (quest_id, display_date, ticket_no, bonus_xp, note, created_by)
    ...
    async deleteQotd(id) {
        const result = await this.database.query('DELETE FROM quest_of_the_day WHERE id = $1', [id]);
        if (!result.rowCount)
            throw new NotFoundException({
                code: 'QOTD_NOT_FOUND',
    ...
    async waitlist(limit, offset) {
        return (await this.database.query('SELECT * FROM waitlist ORDER BY created_at DESC LIMIT $1 OFFSET $2', [limit, offset])).rows;
    }
    async suggestions(status, limit, offset) {
        return (await this.database.query(`SELECT * FROM quest_suggestions WHERE ($1 = 'all' OR status = $1)
       ORDER BY created_at DESC LIMIT $2 OFFSET $3`, [status, limit, offset])).rows;
    }
    async reviewSuggestion(actorId, id, status, xpReward, durationHours) {
        return this.database.transaction(async (transaction) => {
            const suggestion = await transaction.query(`UPDATE quest_suggestions SET status = $2, reviewed_by = $3, reviewed_at = now()
         WHERE id = $1 AND status = 'pending' RETURNING *`, [id, status, actorId]);
           ...
           VALUES ($1, $2, $3, $4, $5, $6, true, $7) RETURNING id`, [
            ...
            await this.audit(actorId, `suggestion.${status}`, 'quest_suggestion', id, null, { quest_id: questId }, transaction);
            return { ...suggestion.rows[0], quest_id: questId };
        });
    }
    async audit(actorId, action, targetType, targetId, before, after, transaction) {
