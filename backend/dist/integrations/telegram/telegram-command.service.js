...
let TelegramCommandService = class TelegramCommandService {
    repository;
    client;
    ...
    async handle(chatId, rawText) {
        ...
        if (!text.startsWith('/')) {
            if (await this.routeState(chatId, text))
                return;
        ...
        switch (command) {
            ...
            case '/help':
                ...
                break;
            case '/broadcast':
                await this.broadcast(chatId, argument);
                break;
            case '/pending':
                await this.pending(chatId);
                break;
            case '/stats':
                await this.stats(chatId, argument);
                break;
            case '/user':
                await this.user(chatId, argument);
                break;
            case '/ban':
                await this.ban(chatId, argument, true);
                break;
            case '/unban':
                await this.ban(chatId, argument, false);
                break;
            case '/quest':
                await this.quest(chatId, argument);
                break;
            case '/audit':
                await this.audit(chatId, argument);
                break;
            case '/leaderboard':
                await this.leaderboard(chatId);
                break;
    ...
    async ban(chatId, argument, shouldBan) {
        if (!argument) {
        ...
        await this.client.sendMessage(chatId, `${shouldBan ? '🚫 Banned' : '✅ Restored'} <b>@${escapeHtml(profile.username)}</b>`);
    }
    async quest(chatId, argument) {
    ...
    async audit(chatId, argument) {
        ...
        await this.client.sendMessage(chatId, [heading, '', ...rows.map((row) => `• <code>${date(row.created_at, 16)}</code> ${escapeHtml(row.actor_name ?? '(system)')} → ${escapeHtml(row.action)}`)].join('\n'));
    }
    async leaderboard(chatId) {
        ...
        await this.client.sendMessage(chatId, [
            ...
            ...rows.map((row, index) => `${medals[index]} ${escapeHtml(row.display_name || `@${row.username}`)} — <b>${row.approvals}</b>`),
        ].join('\n'));
    }
    async routeState(chatId, text) {
    ...
    async questStep(chatId, state, input) {
        const data = { ...state.data };
        switch (state.step) {
            case 'awaiting_title':
                if (input.length < 3 || input.length > 100)
                    return this.say(chatId, 'Title must be 3–100 chars. Try again or /cancel.');
                data.title = input;
            ...
            case 'awaiting_description':
                if (input.length < 10 || input.length > 500)
                    return this.say(chatId, 'Description must be 10–500 chars. Try again or /cancel.');
            ...
            case 'awaiting_category': {
                ...
                if (!QUEST_CATEGORIES.includes(category))
                    return this.say(chatId, `Unknown category. Pick one of: ${QUEST_CATEGORIES.join(' | ')}`);
            ...
            case 'awaiting_difficulty': {
                ...
                if (!QUEST_DIFFICULTIES.includes(difficulty))
                    return this.say(chatId, `Unknown difficulty. Pick one of: ${QUEST_DIFFICULTIES.join(' | ')}`);
            ...
            case 'awaiting_xp': {
                ...
                if (!Number.isFinite(xp) || xp < 5 || xp > 100)
                    return this.say(chatId, 'XP must be an integer 5–100. Try again or /cancel.');
                data.xp_reward = xp;
            ...
            case 'awaiting_confirm': {
                if (input.trim().toLowerCase() !== 'yes')
                    return this.say(chatId, 'Reply YES to create, or /cancel.');
                try {
    ...
    async say(chatId, message) {
        await this.client.sendMessage(chatId, message);
    }
};
TelegramCommandService = __decorate([
    Injectable(),
    __metadata("design:paramtypes", [TelegramRepository,
        TelegramClient])
], TelegramCommandService);
export { TelegramCommandService };
function date(value, length) {
    return new Date(value).toISOString().replace('T', ' ').slice(0, length);
}
//# sourceMappingURL=telegram-command.service.js.map