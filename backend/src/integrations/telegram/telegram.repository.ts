...
export interface TelegramDailySummary {
  ...
  readonly top_quests: readonly { title: string; approvals: number }[];
}
...
export class TelegramRepository {
  ...
  async submission(id: string): Promise<TelegramSubmission | null> {
    const result = await this.database.query<TelegramSubmission>(
      `SELECT submission.id, submission.media_url, submission.media_type::text,
  ...
  async setSubmissionMessageId(id: string, messageId: number): Promise<void> {
    await this.database.query(
      'UPDATE submissions SET telegram_message_id = $2 WHERE id = $1',
      [id, messageId],
    );
  }

  async user(id: string): Promise<TelegramUser | null> {
    const result = await this.database.query<TelegramUser>(
      `SELECT users.id, users.email::text, profile.username::text, profile.display_name,
       ...
       WHERE users.id = $1
       GROUP BY users.id, users.email, profile.username, profile.display_name`,
      [id],
    );
    return result.rows[0] ?? null;
  }

  async report(id: string): Promise<TelegramReport | null> {
    const result = await this.database.query<TelegramReport>(
      `SELECT report.id, report.reported_type, report.reason,
       ...
       WHERE report.id = $1 AND report.status = 'pending'`,
      [id],
    );
    return result.rows[0] ?? null;
  }

  async claimUpdate(updateId: string): Promise<boolean> {
  ...
  async finishUpdate(updateId: string, error?: unknown): Promise<void> {
    ...
    await this.database.query(
      ...
      [updateId, failed ? 'failed' : 'processed', failed ? errorText(error) : null],
    );
  }

  async pendingCounts(): Promise<{ submissions: number; appeals: number; reports: number }> {
    return (await this.database.query<{ submissions: number; appeals: number; reports: number }>(
      `SELECT
  ...
  async stats(days: 1 | 7): Promise<{
    submissions: number; approved: number; rejected: number; signups: number; submitters: number;
  }> {
    return (await this.database.query<{
      ...
      `SELECT
         ...
         (SELECT count(DISTINCT user_id)::integer FROM submissions WHERE submitted_at >= now() - ($1 * interval '1 day')) AS submitters`,
      [days],
    )).rows[0];
  }

  async profileByHandle(handle: string): Promise<TelegramProfileCard | null> {
    const result = await this.database.query<TelegramProfileCard>(
      `SELECT profile.id, profile.username::text, profile.display_name, profile.xp, profile.level,
       ...
       WHERE lower(profile.username::text) = lower($1)`,
      [stripHandle(handle)],
    );
    return result.rows[0] ?? null;
  }

  async setAccountStatus(userId: string, status: 'active' | 'banned', chatId: string): Promise<void> {
    await this.database.transaction(async (transaction) => {
      const before = await transaction.query<{ status: string }>(
        'SELECT status::text FROM users WHERE id = $1 FOR UPDATE', [userId],
      );
      if (!before.rows[0]) throw new Error('User not found');
      await transaction.query(
        `UPDATE users SET status = $2::account_status, token_version = token_version + 1,
      ...
      if (status === 'banned') await transaction.query('DELETE FROM device_tokens WHERE user_id = $1', [userId]);
      await transaction.query(
        `INSERT INTO admin_audit_log (actor_id, action, target_type, target_id, before_state, after_state)
         VALUES (NULL, 'user.set_status', 'user', $1,
                 ...
                 jsonb_build_object('status', $3::text, 'source', 'telegram', 'telegram_chat_id', $4::text))`,
        [userId, before.rows[0].status, status, chatId],
      );
      await transaction.query(
        `INSERT INTO outbox_events (aggregate_type, aggregate_id, event_type, payload)
         VALUES ('user', $1, 'admin.user_status_changed',
                 jsonb_build_object('userId', $1::uuid, 'status', $2::text, 'source', 'telegram'))`,
        [userId, status],
      );
    });
  }

  async broadcast(title: string, body: string, chatId: string): Promise<number> {
    return this.database.transaction(async (transaction) => {
      const result = await transaction.query<{ recipient_count: number }>(
        `WITH inserted AS (
         ...
         SELECT count(*)::integer AS recipient_count FROM emitted`,
        [title.slice(0, 180), body.slice(0, 500)],
      );
        ...
        `INSERT INTO admin_audit_log (actor_id, action, target_type, after_state)
         VALUES (NULL, 'notification.broadcast', 'notification',
                 jsonb_build_object('source', 'telegram', 'telegram_chat_id', $1::text,
                                    'title', $2::text, 'recipient_count', $3::integer))`,
        [chatId, title, recipients],
      );
      return recipients;
    });
  }

  async activeQuests(): Promise<readonly { id: string; title: string; description: string }[]> {
    return (await this.database.query<{ id: string; title: string; description: string }>(
      `SELECT id, title, description FROM quests WHERE is_active
       ORDER BY created_at DESC, id DESC LIMIT 50`,
    )).rows;
  }

  async audit(handle?: string): Promise<readonly {
    created_at: Date; actor_name: string | null; action: string;
  }[]> {
    return (await this.database.query<{
      created_at: Date; actor_name: string | null; action: string;
    }>(
      `SELECT audit.created_at,
       ...
       ORDER BY audit.created_at DESC LIMIT 10`,
      [handle ? stripHandle(handle) : null],
    )).rows;
  }

  async weeklyLeaderboard(): Promise<readonly { display_name: string; username: string; approvals: number }[]> {
    return (await this.database.query<{
      display_name: string; username: string; approvals: number;
    }>(
      `SELECT profile.display_name, profile.username::text,
       ...
       ORDER BY approvals DESC, profile.id LIMIT 5`,
    )).rows;
  }

  async dailySummary(): Promise<TelegramDailySummary> {
    return (await this.database.query<TelegramDailySummary>(
      `WITH top_quests AS (
       ...
       SELECT
         ...
         COALESCE((SELECT jsonb_agg(jsonb_build_object('title', title, 'approvals', approvals)
                   ...
                   FROM top_quests), '[]'::jsonb) AS top_quests`,
    )).rows[0];
  }

  async commandState(chatId: string): Promise<TelegramCommandState | null> {
    const result = await this.database.query<TelegramCommandState>(
      `SELECT chat_id::text, command, step, data FROM telegram_command_state WHERE chat_id = $1::bigint`,
      [chatId],
    );
    return result.rows[0] ?? null;
  }
  ...
  async setCommandState(
    chatId: string, command: string, step: string, data: Record<string, unknown>,
  ): Promise<void> {
    await this.database.query(
      `INSERT INTO telegram_command_state (chat_id, command, step, data)
       VALUES ($1::bigint, $2, $3, $4::jsonb)
       ON CONFLICT (chat_id) DO UPDATE SET command = EXCLUDED.command, step = EXCLUDED.step,
         data = EXCLUDED.data, updated_at = now()`,
      [chatId, command, step, JSON.stringify(data)],
    );
  }

  async clearCommandState(chatId: string): Promise<void> {
    await this.database.query('DELETE FROM telegram_command_state WHERE chat_id = $1::bigint', [chatId]);
  }

  async createQuest(data: Record<string, unknown>, chatId: string): Promise<string> {
    return this.database.transaction(async (transaction) => {
      const quest = await transaction.query<{ id: string }>(
        ...
        [data.title, data.description, data.category, data.difficulty, data.xp_reward],
      );
      await transaction.query(
        `INSERT INTO admin_audit_log (actor_id, action, target_type, target_id, after_state)
         VALUES (NULL, 'quest.create', 'quest', $1,
                 jsonb_build_object('source', 'telegram', 'telegram_chat_id', $2::text))`,
        [quest.rows[0].id, chatId],
      );
  ...
  async reviewReport(
    reportId: string, action: 'dismiss' | 'action', chatId: string,
  ): Promise<{ ok: boolean; message: string }> {
    return this.database.transaction(async (transaction) => {
      const result = await transaction.query<{
        status: string; reported_type: string; reported_id: string; offender_id: string | null;
      }>(
        `SELECT report.status, report.reported_type, report.reported_id,
      ...
      if (report.status !== 'pending') return { ok: false, message: `Already ${report.status}` };
      if (action === 'action' && !report.offender_id) return { ok: false, message: 'Reported user not found' };

      if (action === 'action') {
        await transaction.query(
          `UPDATE users SET status = 'banned', token_version = token_version + 1,
      ...
      await transaction.query(
        `UPDATE reports SET status = $2, reviewed_by = NULL, reviewed_at = now() WHERE id = $1`,
        [reportId, status],
      );
      await transaction.query(
        `INSERT INTO admin_audit_log (actor_id, action, target_type, target_id, after_state)
         VALUES (NULL, $2, 'report', $1,
                 jsonb_build_object('source', 'telegram', 'telegram_chat_id', $3::text,
                                    'status', $4::text, 'offender_id', $5::uuid))`,
        [reportId, action === 'action' ? 'report.action' : 'report.dismiss', chatId, status, report.offender_id],
      );
